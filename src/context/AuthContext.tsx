...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const verifyOtp = async (otp: string) => {
        ...
        // Re-check the phase right here, same reasoning as login(): a voter can
        // sit for a minute or two between requesting a code and typing it in,
        // and the EC may close one phase and open the next in that exact
        // window (e.g. round 1 closes, runoff opens). Using the phase we
        // captured back at login time was sending these voters to the wrong
        // endpoint and leaving them stuck on "Voting is not currently open"
        // even though their code was still valid on the other endpoint.
        const currentPhase = await fetchPhase();
        const verifyEndpoint = currentPhase === 'runoff' ? '/api/runoff/verify-otp' : '/api/verify-otp';
